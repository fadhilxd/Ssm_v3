...
class api_provider extends MX_Controller {
	...
	public function ajax_update($ids = ""){
		...
		if (!empty($api_key) && !empty($api_url)) {
			...
			$data_connect = connect_api($api_url, $data_post);
	...
	public function ajax_update_api_provider($ids){
		if ($ids != "" ) {
			...
			if (!empty($api)) {
				...
				$data_connect = connect_api($api->url, $data_post);
				$data_connect = json_decode($data_connect);
	...
	public function ajax_sync_services($ids){
		...
		if ($ids != "") {
			...
			if (!empty($api)) {
				...
				$data_services = connect_api($api->url, $data_post);
	...
	public function ajax_api_provider_services($ids = ""){
		if (!empty($ids)) {
			...
			if (!empty($api)) {
				...
				if (empty($data_services) || !is_array($data_services)) {
					$message = '<div class="alert alert-icon alert-danger" role="alert"> <i class="far fa-exclamation-triangle mr-2" aria-hidden="true"></i> '.lang("there_seems_to_be_an_issue_connecting_to_api_provider_please_check_api_key_and_token_again").'</div>';
					echo $message;
				}
				$data_columns = array(
