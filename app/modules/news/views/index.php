...
<div class="page-header text-center">
  <h1 class="page-title">
    ...
    ?>
    <a href="<?=cn("$module/update")?>" class="ajaxModal"><span class="add-new" data-toggle="tooltip" data-placement="bottom" title="<?=lang("add_new")?>"><i class="fas fa-plus-square text-primary" aria-hidden="true"></i></span></a> 
    <?php }else{?>
    <span><i class="far fa-question-circle" aria-hidden="true"></i></span>
    <?php }?>
              ...
              <td class="text-center">
                <div class="item-action dropdown">
                  ...
                  <div class="dropdown-menu">
                    <a href="<?=cn("$module/ajax_notification/")?>" class="dropdown-item ajaxModal"><i class="dropdown-icon far fa-eye"></i> <?=lang("View")?> </a>

                    <a href="<?=cn("$module/update/".$row->ids)?>" class="dropdown-item ajaxModal"><i class="dropdown-icon far fa-edit"></i> <?=lang('Edit')?> </a>
                    <?php
                      if (get_role('admin')) {
                    ?>
                    <a href="<?=cn("$module/ajax_delete_item/".$row->ids)?>" class="dropdown-item ajaxDeleteItem">  <i class="dropdown-icon far fa-trash"></i> <?=lang('Delete')?> 
                    </a>
                    <?php }?>
                  </div>
                </div>
